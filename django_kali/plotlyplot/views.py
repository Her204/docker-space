...
def plot_fao(request):
    ...
    file_path = os.getcwd()+"/plotlyplot/objects-django_kali"
    geoplot_load = load(open(file_path+"/plot_geografico.pkl","rb"))
    img_load = load(open(file_path+"/imagen_dinamica.pkl","rb"))
    pie_load = load(open(file_path+"/tartara_plot.pkl","rb"))
    graph_load = load(open(file_path+"/red_de_grafos.pkl","rb"))
    ...
    #]
    functions = [
        "geoplot_load","table()","pie_load",
        "box_2()","img_load","graph_load"
    ]
    q_vals = [
        "plot_geografico",
        "enseñar_tablas",
